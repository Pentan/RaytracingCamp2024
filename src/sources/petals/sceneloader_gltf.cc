...
namespace {
    // Constants
    std::string kExtraPrefix = "Petals-Extra-";
    ...
    // Utility functions
    tinygltf::Value FindPetalsExtra(const tinygltf::Value& val, const std::string& targetKey) {
        if(val.IsArray()) {
            size_t n = val.ArrayLen();
            for(size_t i = 0; i < n; i++) {
                auto v = FindPetalsExtra(val.Get(static_cast<int>(i)), targetKey);
            ...
            for(auto ite = keys.begin(); ite != keys.end(); ++ite) {
                ...
                if(k.compare(targetKey) == 0) {
                    ...
                    if(v.IsObject() || v.IsArray()) {
                        return FindPetalsExtra(v, targetKey);
                    }
                }
            }
        }
    ...
    std::string getPetalsExtraKey(std::string key) {
    ...
    int ParseTextures(const tinygltf::Model& model, AssetLibrary *assetlib) {
        ...
        for(auto ite = model.textures.begin(); ite != model.textures.end(); ++ite) {
            ...
            double gamma = 2.2;
            auto ppextra = FindPetalsExtra(gltfimg.extras, getPetalsExtraKey("gamma"));
    ...
    int ParseMaterials(const tinygltf::Model& model, AssetLibrary *assetlib) {
        ...
        for(auto ite = model.materials.begin(); ite != model.materials.end(); ++ite) {
            ...
            // extra
            auto ppextra = FindPetalsExtra(gltfmat.extras, getPetalsExtraKey("ior"));
            if (ppextra.Type() != tinygltf::NULL_TYPE) {
    ...
    */
    ...
    int ParseCameras(const tinygltf::Model& model, AssetLibrary *assetlib) {
        ...
        for(auto ite = model.cameras.begin(); ite != model.cameras.end(); ++ite) {
            ...
            //ppextra = FindPetalsExtra(gltfcam.extras, getPetalsExtraKey("focalLength"));
            //if (ppextra.Type() != tinygltf::NULL_TYPE) {
            //    cam->focalLength = ppextra.Get<RTFloat>();
            //}
            ppextra = FindPetalsExtra(gltfcam.extras, getPetalsExtraKey("fNumber"));
            if (ppextra.Type() != tinygltf::NULL_TYPE) {
                cam->fNumber = ppextra.Get<RTFloat>();
            }
            ppextra = FindPetalsExtra(gltfcam.extras, getPetalsExtraKey("focusDistance"));
            if (ppextra.Type() != tinygltf::NULL_TYPE) {
    ...
    int ParseScenes(const tinygltf::Model& model, AssetLibrary *assetlib) {
        ...
        for(auto ite = model.scenes.begin(); ite != model.scenes.end(); ++ite) {
            ...
            auto ppextra = FindPetalsExtra(gltfscene.extras, getPetalsExtraKey("background"));
            if (ppextra.Type() != tinygltf::NULL_TYPE) {
